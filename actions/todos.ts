...
export async function updateTodo(userId: string, todoId: string, updatedTodo: { todo: string }) {
    try {
        const result = await sql`UPDATE todos SET todo = ${updatedTodo.todo} WHERE id = ${todoId} AND user_id = ${userId} RETURNING *`
...
export async function deleteTodo(id: string) {
    try {
        ...
        if (result.rows && result.rows.length > 0) {
            ...
            return { message: 'Não foi possível excluir a tarefa' }
        }
    } catch (error) {
